...
function randBook(options) {
  const bookData = options != null && options.category ? data$1O.filter(({
    category
  }) => category === options.category) : data$1O;
...
function randCreditCardIssuer(options) {
...
function randCreditCardNumber(options) {
  let formats = data$1C.map(({
    formats
  }) => formats).flat();

  if (options != null && options.issuer) {
    var _data$find;

    formats = ((_data$find = data$1C.find(card => {
...
function randFirstName(options) {
  ...
  const names = withAccents ? data$1m['withAccents'] : data$1m['withoutAccents'];
...
function randLastName(options) {
  ...
  const withAccents = (_options$withAccents = options == null ? void 0 : options.withAccents) != null ? _options$withAccents : randBoolean();
  const names = withAccents ? data$1l['withAccents'] : data$1l['withoutAccents'];
  return fake(names, options);
}
...
function randFileName(options) {
  ...
  return fake(() => `${rand(data$1i)}.${ext}`, options);
}
...
function randGender(options) {
  return fake(options != null && options.code ? data$1d : data$1e, options);
}
...
function randPhoneNumber(options) {
  ...
  if (options != null && options.countryCode) {
    ...
    formats = ((_data$find = data$Y.find(country => {
...
function randProtocol(options) {
  return fake(options != null && options.fullName ? data$P : data$Q, options);
}
...
function randLanguage(options) {
  return fake(options != null && options.code ? data$s : data$t, options);
}
...
var _Object$keys$1;
const totalOrigins = (_Object$keys$1 = Object.keys(data$q)) == null ? void 0 : _Object$keys$1.length;
/**
...
function randStatus(options) {
  ...
  if (options != null && options.type) {
    const filteredStatuses = data$a.filter(status => {
...
function randSuperhero(options) {
  const factory = () => {
    const heroes = options != null && options.company ? data$6.filter(({
      company
    }) => company === options.company) : data$6;
    return Object.assign({}, rand(heroes), {
      id: randUuid()
    });
  };
...
var _Object$keys;
const categoriesCount = (_Object$keys = Object.keys(data$5)) == null ? void 0 : _Object$keys.length;
/**
...
export { rand, randAbbreviation, randAccessory, randAccount, randAlpha, randAlphaNumeric, randAmericanFootballTeam, randAnimalType, randArn, randAvatar, randAwsRegion, randAwsService, randBaseballTeam, randBasketballTeam, randBear, randBetweenDate, randBinary, randBird, randBitcoinAddress, randBook, randBoolean, randBrand, randBrowser, randCardinalDirection, randCat, randCatchPhrase, randCetacean, randCity, randClothingSize, randColor, randCompanyName, randCountry, randCountryCode, randCounty, randCow, randCreditCardCVV, randCreditCardIssuer, randCreditCardNumber, randCrocodilia, randCurrencyCode, randCurrencyName, randCurrencySymbol, randDatabaseCollation, randDatabaseColumn, randDatabaseEngine, randDatabaseType, randDepartment, randDirection, randDirectoryPath, randDog, randDomainName, randDomainSuffix, randDrinks, randEmail, randEmailProvider, randEmoji, randEthereumAddress, randFileExt, randFileName, randFilePath, randFileType, randFirstName, randFish, randFloat, randFontFamily, randFontSize, randFood, randFootballTeam, randFrequency, randFullName, randFutureDate, randGender, randGitBranch, randGitCommitEntry, randGitCommitMessage, randGitCommitSha, randGitShortSha, randHex, randHexaDecimal, randHorse, randHsl, randHttpMethod, randIban, randIceHockeyTeam, randImg, randIntegration, randIp, randIpv6, randJobArea, randJobDescriptor, randJobTitle, randJobType, randLanguage, randLastName, randLatitude, randLine, randLines, randLion, randLocale, randLongitude, randMac, randMask, randMimeType, randMonth, randMovie, randMovieCharacter, randMusicGenre, randNearbyGPSCoordinate, randNumber, randOAuthProvider, randOctal, randOrdinalDirection, randParagraph, randPassword, randPastDate, randPermission, randPersonTitle, randPhoneNumber, randPhrase, randPort, randPost, randPriority, randProduct, randProductAdjective, randProductCategory, randProductDescription, randProductMaterial, randProductName, randProgrammingLanguage, randPronoun, randProtocol, randQuote, randRabbit, randRecentDate, randRgb, randRole, randRoutingNumber, randSemver, randSentence, randSequence, randShape, randSkill, randSlug, randSnake, randSocial, randSoonDate, randSports, randSportsTeam, randState, randStateAbbr, randStatus, randStreetAddress, randStreetName, randSubscriptionPlan, randSuperhero, randSuperheroName, randSvg, randText, randTimeZone, randTodo, randTransactionType, randUrl, randUser, randUserAgent, randUserName, randUuid, randVehicle, randVehicleFuel, randVehicleManufacturer, randVehicleModel, randVehicleType, randVerb, randWeekday, randWord, randZipCode, random, seed };
